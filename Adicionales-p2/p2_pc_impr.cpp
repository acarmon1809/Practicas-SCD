...
class ProdConsMu : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   multiplos_impresos,
   multiplos_anteriores; //Multiplos de 5 que había en la llamada anterior
...
ProdConsMu::ProdConsMu(  )
{
   ...
   multiplos_anteriores = 0;
   ocupadas      = newCondVar();
   libres        = newCondVar();
   m_impresos_cond = newCondVar();
}
// -----------------------------------------------------------------------------
...
bool ProdConsMu::metodo_impresora(){
   ...
   int nuevos_multiplos = multiplos_impresos-multiplos_anteriores;
   if(nuevos_multiplos == 0) m_impresos_cond.wait();
   nuevos_multiplos = multiplos_impresos-multiplos_anteriores;
   
   
   if(multiplos_impresos <= num_items/5){
      mtx.lock();
      cout << "Multiplos impresos: " << nuevos_multiplos << endl;
      mtx.unlock();
      multiplos_anteriores = multiplos_impresos;
      
      if(multiplos_impresos == num_items/5) return false;
   }
   else return false;
   return true;
}
...
void funcion_hebra_productora( MRef<ProdConsMu> monitor, int n_hebra)
{
...
void funcion_hebra_consumidora( MRef<ProdConsMu>  monitor, int n_hebra )
{
...
int main()
{
   ...
   for(int j = 0; j < n_c; j++)
      hebras_consumidoras[j] = thread(funcion_hebra_consumidora, monitor, j);
