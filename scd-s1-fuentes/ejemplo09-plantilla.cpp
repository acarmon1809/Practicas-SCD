...
// -----------------------------------------------------------------------------
// función que ejecuta cada hebra: recibe $i$ ==índice de la hebra, ($0\leq i<n$)     //Cada funcion_hebra evalua f en m/n_puntos del intervalo m numero de hebras
double funcion_hebra( long i )
{
   double suma = 0.0;

   //Calculamos la suma en los puntos que le corresponden
   for(long punto = (i-1)*m/n; punto < i*m/n; punto++){
      suma += f((punto+0.5)/m);
   }
...
{
  double suma_total = 0;
  future<double> future_i[n];

  for(int hebra = 1; hebra <= n; hebra++)
      future_i[hebra-1] = async( launch::async, funcion_hebra, hebra);
   
  for(int hebra = 1; hebra <= n; hebra++)
      suma_total += future_i[hebra-1].get();
